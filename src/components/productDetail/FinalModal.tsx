...
import {
  useAuctionPutStatus,
  useGetAuctionProductDetail,
  useGetAuctionProducts,
  useUpdateProduct,
} from "@/api/productApi";
...
const FinalModal = ({ auctionId }: FinalModalProps) => {
  ...
  const handleMovePage = () => {
    if (data?.data.nickname === winner) {
      activeStatus(
        { auctionId: auctionId, status: false, isActive: "결제대기" },
        {
          onSuccess: () => {
            productStatusUpdate(
              {
            ...
            productStatusUpdate(
              ...
              },
              {
                onSuccess: () => {
                  // itemRefetch();
                  router.push(
                    `/productList/detail/?id=${randomItem?.product_id}&productId=${randomItem?.id}`
                  );
                },
              }
            );
        ...
        <div className="w-[400px]  rounded-xl bg-[#242424] border border-[#D1B383]  p-5 transition-all duration-[0.3s] ease-out animate-scale-up">
          <div className="text-4xl text-center text-white">
            <div className="flex flex-col items-center justify-center">
              ...
              <div
                className={`w-[220px] h-[30px] mt-2 relative ${
                  data?.data.nickname === winner ? "block" : "hidden"
                }`}
              >
                <Image
                  src={
                    data?.data.nickname === winner
                      ? "/images/detailModal01.png"
                      : "/images/detailModal01.png"
                  }
                  fill
                  sizes="1"
            ...
            <div className="flex items-center bg-white rounded-xl p-4">
              <div className="w-[100px] h-[100px] rounded-xl object-cover bg-[gray] relative overflow-hidden">
                {data?.data.nickname === winner
                  ? detailData?.data.product_images[0] && (
                      <Image
                        src={detailData?.data.product_images[0]}
                        fill
                        sizes="1"
                  ...
                  : randomItem?.product_images[0] && (
                      <Image
                        src={randomItem?.product_images[0]}
                        fill
                        sizes="1"
                        className="object-cover"
                        alt="모달 이미지"
              ...
              <div className="ml-3">
                <div className="flex items-center font-bold text-lg">
                  <p className="mr-2">
                    {data?.data.nickname === winner
                      ? detailData?.data.product_grade
                      : randomItem?.product_grade}
                  </p>
                  <p>
                    {data?.data.nickname === winner
                      ? detailData?.data.product_name
                      : randomItem?.product_name}
                  </p>
                </div>
                <div className="mt-5">
                  <p className="mr-2 text-[#868686]">
                    {data?.data.nickname === winner
                      ? "최종 입찰가"
                      : "현재 입찰가"}
                  </p>
                  <p>
                    {data?.data.nickname === winner
                      ? detailData?.data.final_price.toLocaleString()
                      : randomItem?.final_price.toLocaleString()}
                    원
                  </p>
                </div>
              </div>
          ...
          <div className="text-center mt-10">
            <button
              className="w-[200px] h-[50px] rounded-xl outline-none bg-[#D1B383] hover:bg-white border border-[#D1B383] text-white hover:text-[#D1B383]"
              onClick={handleMovePage}
            >
              {data?.data.nickname === winner ? "결제하기" : "입찰하기"}
            </button>
          </div>
        </div>
      </div>
