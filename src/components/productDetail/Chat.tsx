  ...
  return (
    <div className="w-[568px] max-[1255px]:w-full px-[12px] max-[1255px]:px-0 max-[1255px]:pr-3 max-[855px]:pr-0 m-3 max-[855px]:m-0 max-[855px]:my-3">
            ...
            {messages.map((chat, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`${
                    chat.includes(data?.data.nickname)
                      ? "text-end"
                      : "text-start"
                  } py-4 rounded-xl p-3 my-1`}
                >
                  <p
                    className={` border ${
                      mat
                        ? "block text-center bg-[red] text-white"
      ...
      <div className="flex w-full">
        <input
          type="text"
          className="w-full outline-none h-[50px] border rounded-bl-xl pl-3"
          placeholder="입력"
          value={isChat}
        ...
        />
        <button
          className="w-[200px] h-[50px] bg-[#D1B383] text-white rounded-br-xl"
          onClick={() => {
            if (!localStorage.getItem("access_token")) {
              alert("로그인이 필요합니다");
              return;
      ...
      <div className="flex justify-between max-[1255px]:justify-evenly max-[640px]:justify-between items-center my-4">
        {buttonMenu.map((item) => (
          <div
            key={item.id}
            className="w-[170px] h-[48px] max-[640px]:w-[145px] cursor-pointer bg-[#D1B383] border border-[#D1B383] hover:bg-white hover:text-[#D1B383] flex justify-center items-center text-white rounded-[10px]"
